...
public class MenuManager : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        temp.transform.localPosition = bgObjs[ran].position; //Vector2.zero;
        temp.transform.localScale = Vector3.one;

        if(bgObjs[ran].particle != null) // 나중에 지우기 조건 연상 비용 큼..
    ...
    public void CheckSoundOptionPanelInput()
    {
        ...
        else if (CGameManager.rewiredPlayer1.GetAxisRaw("Move Horizontal") > 0 && Input_on)
        {
            ...
            if (is_bgmOptionOn)
            {
                ...
                //CGameManager.bgmVolume = bgmVol;
                //CGameManager.bgmAudioSource.volume = bgmVol;
                Bgm_Slider.value = bgmVol;
            }
            else if (is_sfxOptionOn)
            {
                ...
                //CGameManager.soundFXVolume = sfxVol;
                Sfx_Slider.value = sfxVol;
            }
            Input_on = false;
            timer = 0.0f;
